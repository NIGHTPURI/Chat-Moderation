...
public final class OpenAiProviderTelemetry {
    ...
    void recordSuccess(long input, long cachedInput, long output) {
        successes.increment();
    ...
    public record Snapshot(
        ...
        public String format() {
            return String.format(Locale.ROOT,
                    "requests=%d, success=%d, error=%d, timeout=%d%n"
                            + "input tokens=%d (cached=%d), output tokens=%d%n"
                            + "latency average=%.3f ms, p50=%.3f ms, p95=%.3f ms, p99=%.3f ms",
                    requests, successes, errors, timeouts,
